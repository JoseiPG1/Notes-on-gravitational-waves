...
\end{equation}
vanishes the vast majorities of terms of Eq.\eqref{efe X}. The only, non-trivial term 
comes from the wave operator, leading to
\begin{equation}
    \square X_{\mu\nu} = -\frac{16\Pi G}{c^4}\mathcal{T}_{\mu\nu},
\end{equation}
which for the special vacuum case $\mathcal{T}_{\mu\nu} = 0$, reduces the above equation
to 
\begin{equation}
    \square X_{\mu\nu} = 0.
\end{equation}
